  ...
  <div className="row justify-content-md-center">
    <Form className="col-md-6">
      <fieldset className="form-group">
        ...
        {touched.oldPassword && errors.oldPassword && <small className="form-text text-danger">{errors.oldPassword}</small>}
      </fieldset>
      <fieldset className="form-group">
        <label>New Password</label>
        <Field className="form-control" type="password" name="newPassword" placeholder="New Password"/>
        {touched.newPassword && errors.newPassword && <small className="form-text text-danger">{errors.newPassword}</small>}
      </fieldset>
      <fieldset className="form-group">
        <label>Confirm New Password</label>
        <Field className="form-control" type="password" name="passwordConfirmation"
               placeholder="New Password Confirmation"/>
        {touched.passwordConfirmation && errors.passwordConfirmation && <small className="form-text text-danger">{errors.passwordConfirmation}</small>}
      </fieldset>
...
const EnhancedForm = withFormik({
  ...
  async handleSubmit(values, {props, resetForm, setFieldError, setSubmitting, setStatus}) {
    setStatus(null);
    try {
      await props.settingsChangePassword(values);
      setSubmitting(false);
      resetForm();
      setStatus({'success': 'Your password has been updated successfully!'});
    } catch (errors) {
